...
O alistamento manual permite que você **submeta um pedido de entrada em um clã** diretamente pelo app, sem precisar acessar o site do Painel separadamente.

::: warning Funcionalidade opcional
O alistamento manual pode estar **desativado** pelo administrador do clã. Quando isso acontece, a tela fica **inacessível para membros comuns** — não é uma versão limitada, você é bloqueado de entrar nela. Se não conseguir acessar, entre em contato com um Admin.

**Admin e Gerenciador de Clã** continuam vendo a tela normalmente mesmo com a funcionalidade desativada. Nesse caso, porém, tocar no clã não abre o fluxo de alistamento manual — leva direto para **Ver Membros**, já que a mesma tela/lista serve para as duas finalidades.
:::
...
Você precisará das suas **credenciais do Painel** (e-mail e senha da conta Painel, não do app Timaceta). É com elas que o sistema faz o pedido de alistamento em seu nome.
...
Se o Painel demorar mais de 6 segundos para responder, uma mensagem aparecerá: *"O Painel está demorando..."*. Aguarde — o timeout é de 60 segundos.
:::
...
Se o personagem selecionado não estiver cadastrado no Timaceta, você verá um aviso. Você pode continuar mesmo assim, mas o **alistamento automático irá rejeitá-lo** até que o personagem seja cadastrado no app.
:::
...
## Atalhos para Admin e Gerenciador de Clã

Na mesma tela de alistamento, quem tem cargo de **Admin** ou **Gerenciador de Clã** encontra, por clã, dois atalhos adicionais:

- **Ver Logs do Alistamento:** histórico das decisões (aceitos, rejeitados, pendentes) tomadas pelo sistema
- **Regras de Alistamento** (ícone de informação): resumo das regras configuradas para aquele clã
...
Na maioria dos casos, não. O automático verifica a fila do Painel a cada 5 minutos. O manual é útil quando você quer garantir que o pedido foi enviado, ou se a liderança pediu para usar este método.
